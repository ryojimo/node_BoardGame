...
const DataBoardGame = require('./js/DataBoardGame');
...
// 起動の処理関数
//-----------------------------------------------------------------------------
let g_path_top = '/home/pi/workspace/node_BoardGame/';
//let g_path_top = '/home/ec2-user/workspace/node_BoardGame/';
let g_aws_key    = './data/aws_rootkey_bt.json';
let g_aws_region = 'ap-northeast-1';
let g_aws_s3     = 'uz.boardgame.2020';
...
function startSystem() {
  ...
  // bucket 'uz.boardgame.2020' を新規作成する
  // g_apiAws.createBucket(g_aws_s3);

  // AWS からボードゲーム一覧の csv ファイルを取得する
  g_apiAws.download(g_path_top + 'data/', 'bt_boardgames_one.csv', g_aws_s3);
//  g_apiAws.download(g_path_top + 'data/', 'bt_boardgames_many.csv', g_aws_s3);

  // ボードゲーム一覧の csv ファイルを読み出して json 形式の配列データを取得する
...
function setOne() {
  ...
  csv().fromFile('./data/bt_boardgames_one.csv').then((jsonObj)=> {
...
function setMany() {
  ...
  csv().fromFile('./data/bt_boardgames_many.csv').then((jsonObj)=> {
//    console.log("[main.js] jsonObj = " + JSON.stringify(jsonObj));
    g_jsonMany = jsonObj;
  });
}
...
/**
 * json を DataBoardGame オブジェクト配列にセットする
 * @param {object} json - ボードゲーム情報の json 形式の配列データ
 * @param {object} array - DataBoardGame オブジェクトの配列
 * @return {void}
 * @example
 * setArrayBoardGame(g_jsonOne, g_arrayObjOne);
*/
function setArrayBoardGame(json, array) {
  console.log("[main.js] setArrayBoardGame()");
  if(json != null && array.length == 0) {
    for(let value of json) {
      let obj = new DataBoardGame(value);
      array.push(obj);
    }
  }
}
 ...
 * @return {void}
 * @example
 * updateArrayBoardGame(g_arrayObjOne);
*/
function updateArrayBoardGame(array) {
  console.log("[main.js] updateArrayBoardGame()");
...
//  io.sockets.emit('S_to_C_DATA', {value:'user disconnected'});
  ...
  socket.on('C_to_S_INIT', function() {
    ...
    // Array オブジェクトに DataBoardGame オブジェクトをセット
    setArrayBoardGame(g_jsonOne, g_arrayObjOne);
//    setArrayBoardGame(g_jsonMany, g_arrayObjMany);
    setArrayBoardGame(g_jsonRent, g_arrayObjRent);

    // 既に貸出済みのデータがあれば、g_arrayObjOne, g_arrayObjMany の内容を更新する
    updateArrayBoardGame(g_arrayObjOne);
//    updateArrayBoardGame(g_arrayObjMany);

    // DataBoardGame オブジェクト配列から、json データの配列を生成する
    let one = new Array();
    let many = new Array();
...
/**
 * 更新する。
 * @param {object} value - DataBoardGame オブジェクト
